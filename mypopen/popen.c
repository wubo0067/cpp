 ...
 * @Last Modified time: 2021-10-12 17:38:52
 */
...
static inline int custom_popene( const char* command, volatile pid_t* pidptr, char** env, uint8_t flags, FILE** fpp ) {
	...
	if ( !posix_spawn( &pid, "/bin/sh", &actions, &attr, spawn_argv, env ) ) {
		*pidptr = pid;
		...
		error( "Failed to spawn command '%s' from parent pid %d, posix_spawn() failed reason: %s", command, getpid(),
		    strerror( errno ) );
		// 子进程创建失败，关闭pipe的读
		if ( flags & FLAG_CREATE_PIPE ) {
			fclose( fp );
	...
	if ( ret != -1 ) {
		...
		switch ( info.si_code ) {
			...
			case CLD_CONTINUED:
				...
				return 0;
			default:
				error( "child pid %d gave us a SIGCHLD with code %d and status %d", info.si_pid, info.si_code,
				    info.si_status );
				return -5;
		}
	}
...
int32_t mypclose( FILE* fp, pid_t pid ) { return custom_pclose( fp, pid ); }
