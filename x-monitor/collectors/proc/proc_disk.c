...
static enum disk_type __get_disk_type(const char *dev_name, uint32_t major,
                                      uint32_t minor) {
...
static struct io_device *__get_device(uint32_t major, uint32_t minor,
    ...
    dev->minor = minor;
    dev->device_hash = hash;
    dev->dev_tp = __get_disk_type(device_name, major, minor);

    for (index = 0; index < 2; index++) {
        if ((dev->stats[index] = (struct io_stats *)calloc(
                 1, sizeof(struct io_stats))) == NULL) {
            exit(-1);
        }
    }
...
int32_t collector_proc_diskstats(int32_t update_every, usec_t dt) {
    ...
    for (l = 0; l < pf_diskstats_lines; l++) {
        ...
        debug(
            "diskstats: rd_ios=%llu, rw_ios=%llu, rd_merges=%llu, wr_merges=%llu, rd_sectors=%llu, wr_sectors=%llu, rd_ticks=%llu, wr_ticks=%llu, ios_pgr=%llu, tot_ticks=%llu, rq_ticks=%llu",
